...
export default function AdminDashboardPage({ requests, handleStatusUpdate, handleLogout }) {
  return (
    <div className="container py-4">
      <h2 className="mb-4">Admin - All Travel Requests</h2>
              ...
              {req.status === "Pending" && (
                <div className="mt-2">
                  <button className="btn btn-success me-2"
                    onClick={() => handleStatusUpdate(req.id, "Accepted")}>Accept</button>
                  <button className="btn btn-danger"
                    onClick={() => handleStatusUpdate(req.id, "Rejected")}>Reject</button>
                </div>
              )}
            </div>
          ))}
